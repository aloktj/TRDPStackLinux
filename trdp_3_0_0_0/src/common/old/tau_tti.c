 ...
 */
/*
* $Id: tau_tti.c 1997 2019-08-15 12:13:07Z bloehr $
*
    ...
    UINT8               *pData)
{
    ...
    //AÖ copy opTrnTopoCnt is not working, opVehList already allocated for 63 vehicles
    ...
    UINT32              dataSize)
{
    ...
    //AÖ fix, shall we support no closed consist?
    ...
    UINT32              dataSize)
{
    ...
    //AÖ fix
    appHandle->pTTDB->noOfCachedCst = curEntry + 1;
}
    ...
    TRDP_LABEL_T        *pCstId)
{
    ...
    //AÖ fix, entry 0 is always own cst
    if(appHandle->pTTDB->cstInfo[0] == NULL)
    {    /* not found, get it and return immediately */
        ...
        for (index = 0; index < appHandle->pTTDB->cstInfo[0]->fctCnt; index++)
        {
            if (ownIP == appHandle->pTTDB->cstInfo[0]->pFctInfoList[index].fctId && appHandle->pTTDB->cstInfo[0]->pFctInfoList[index].grp == 0)
            {
                ...
                if (pVehId != NULL)
                {
                    ...
                    //AÖ fix number to index (-1)
                    memcpy(pVehId, appHandle->pTTDB->cstInfo[0]->pVehInfoList[vehNo - 1].vehId, TRDP_MAX_LABEL_LEN);
                }
                break;
            }
        }
